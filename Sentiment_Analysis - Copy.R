...
# load tidytext library and textdata dataset which contains Jane Austen's books
library(tidytext)
install.packages("textdata") 

library(janeaustenr)  # dataset of jane austen's books
library(dplyr)
library(stringr)
library(tidyr)

# Dataframe of austen's books
...
# Adding additional stopwords such as "miss"
custom_stop_words <- bind_rows(data_frame(word = c("miss"),
                                          lexicon = c("custom")),
                               stop_words)
custom_stop_words

# installing and loading wordcloud library
...
tidy_books %>%
  ...
  count(word) %>%    # count of each words 
  with(wordcloud(word, n, max.words = 100)) # show only top 100 words 
